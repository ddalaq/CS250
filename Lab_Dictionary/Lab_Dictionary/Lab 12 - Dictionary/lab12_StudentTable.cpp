...
int StudentTable::HashFunction( int key )
{
	...
	while (m_students[index].key != -1)
	{
		...
		{
			QuadraticProbe(originalIndex, quadraticAdd);
		...
		if (m_collisionMethod == DOUBLE)
		{
			...
			doubeHashAdd += HashFunction2(originalIndex);
			index = (originalIndex + doubeHashAdd) % TABLE_SIZE;
		}
	}
