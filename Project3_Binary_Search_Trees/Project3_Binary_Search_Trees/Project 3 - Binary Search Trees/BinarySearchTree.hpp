...
private:
    ...
    */
    Node<TK, TD>* FindParentOfNode( const TK& key )
    {
		...
		while (node)
		{
			...
			{
				if (key < m_ptrRoot->key)
				{
					node = node->ptrLeft;
				}
				else if (key > m_ptrRoot->key)
				{
					node = node->ptrRight;
				}
			}
    ...
    */
    ...
    {
		Node<TK, TD> * leftHeight = m_ptrRoot;
		Node<TK, TD> * rightHeight = m_ptrRoot;
		if (ptrCurrent == nullptr)
		{
			return 0;
		}
		else if (ptrCurrent == ptrCurrent->ptrLeft)
		{
			return GetHeight(leftHeight) + 1;
		}
		else if (ptrCurrent == ptrCurrent->ptrRight)
		{
			return GetHeight(rightHeight) + 1;
		}
		else if (ptrCurrent == ptrCurrent->ptrLeft && ptrCurrent == ptrCurrent->ptrRight)
		{
			GetHeight(leftHeight);
			GetHeight(rightHeight);

			if (GetHeight(leftHeight) > GetHeight(rightHeight))
			{
				return GetHeight(leftHeight) + 1;
			}
			else if (GetHeight(rightHeight) > GetHeight(leftHeight))
			{
				return GetHeight(rightHeight) + 1;
			}
		}
